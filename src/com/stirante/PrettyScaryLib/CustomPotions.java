...
public class CustomPotions {
	 ...
	 */
	public static ItemStack addCustomEffect(ItemStack item,
			PotionEffectType type, int amplifier, int duration) {
	...
	public static CustomEffect[] getCustomEffects(ItemStack item) {
		...
		for (int i = 0; i < list.size(); i++)
			effects.add(new CustomEffect(PotionEffectType
					.getById(((NBTTagCompound) list.get(i)).getByte("Id")),
					((NBTTagCompound) list.get(i)).getByte("Amplifier"),
					((NBTTagCompound) list.get(i)).getInt("Duration")));
